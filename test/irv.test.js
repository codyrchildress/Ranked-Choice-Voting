...
import { tallyIrv } from '../server/methods/irv.js';

test('first-round majority wins immediately', () => {
  const result = tallyIrv(['a', 'b', 'c'], [['a'], ['a'], ['a'], ['b'], ['c']]);
  assert.equal(result.method, 'irv');
...
test('trailing candidate wins after elimination transfers', () => {
  ...
  const result = tallyIrv(['a', 'b', 'c'], ballots);
...
test('zero-vote candidates are eliminated together in one round', () => {
  ...
  const result = tallyIrv(['a', 'b', 'c', 'd', 'e'], ballots);
...
test('elimination ties break by comparing earlier rounds', () => {
  ...
  const result = tallyIrv(['a', 'b', 'c', 'd'], ballots);
...
test('full ties fall back to a seeded random draw, deterministically', () => {
  const ballots = [['a'], ['b'], ['c']];
  const first = tallyIrv(['a', 'b', 'c'], ballots, { seed: 'election-1' });
  const again = tallyIrv(['a', 'b', 'c'], ballots, { seed: 'election-1' });
